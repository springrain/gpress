...
// 注册分词器
func init() {
	registry.RegisterTokenizer(configs.COMMA_ANALYZER_NAME, commaTokenizerConstructor)
	registry.RegisterAnalyzer(configs.COMMA_ANALYZER_NAME, commaAnalyzerConstructor)
}
...
func commaAnalyzerConstructor(interfaceMap map[string]interface{}, cache *registry.Cache) (analysis.Analyzer, error) {
	commaTokenizer, err := cache.TokenizerNamed(configs.COMMA_ANALYZER_NAME)
	if err != nil {
		return nil, err
	}
