...
## 介绍  
Web3内容平台,Hertz + Go template + FTS5全文检索,支持以太坊、索拉纳和百度超级链,兼容Hugo、WordPress生态,使用Wasm扩展插件,只需200M内存  
...
**作为Web3：** gpress已支持以太坊、索拉纳和百度超级链账户体系,会基于Wasm持续迭代去中心功能,让数据自由一点点......  
...
## PostgreSQL 
gpress默认使用sqlite数据库,v1.1.9之后,支持 PostgreSQL 数据库, 必须安装全文检索pg_search插件, 数据库配置在```gpressdatadir/db.json```文件,具体参见[zorm](https://gitee.com/chunanyong/zorm)的配置,例如:
```json
{
...
location ~* \.md$ {
    ...
    # text/plain 比较通用, text/markdown 支持的不好
